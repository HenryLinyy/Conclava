...
Conclava 是本機 **Tool-aware Fusion Agent Gateway**，用一個 8088 gateway 統一 Codex、Claude Code、Hermes Desktop 三種 client，後端接 Ollama 與 ds4。它不是模型訓練、權重融合、MergeKit，也不是 LangChain/LlamaIndex 類 agent framework。
...
```text
Codex:
  conclava, conclava-fast, conclava-full, conclava-heavy
  conclava-fusion  # G10: deliberation router

Claude Code:
  claude-conclava, claude-conclava-fast, claude-conclava-full, claude-conclava-heavy
  claude-conclava-fusion  # G10: deliberation router

Hermes Desktop:
  conclava-chat, conclava-fast, conclava-full, conclava-heavy
  conclava-fusion  # G10: deliberation router
```

## Fusion Deliberation Mode (Gate G10)

`conclava-fusion` / `claude-conclava-fusion` 觸發 **OpenRouter-style multi-model deliberation router**：
...
```python
from conclava.fusion_presets import PRESETS, resolve_preset
...
1. `CONCLAVA_PREFER_MLX_FORMATTER=true`（預設）
2. `tools == []`（沒有 tool calls）
3. `len(text.strip()) < CONCLAVA_MLX_FORMATTER_MAX_CHARS`（預設 1000 字）
...
✗ CONCLAVA_PREFER_MLX_FORMATTER=false
```
...
`conclava-fusion` / `claude-conclava-fusion` 加上 `stream: true` 會啟用 SSE 串流：
...
LM Studio profiles are still serialized by Conclava's global request lock. The compatibility client uses LM Studio's OpenAI-compatible `/v1/chat/completions`; for vision, it translates the previous Ollama-native image shape into OpenAI multimodal `image_url` content. The existing aliases remain available: `conclava-agentic-mlx` maps to `MODEL_AGENTIC_MLX`, and `conclava-formatter-mlx` maps to `MODEL_FORMATTER_MLX`.
...
```toml
model = "conclava-fast"
model_provider = "conclava"

[model_providers.conclava]
name = "Conclava"
base_url = "http://127.0.0.1:8088/v1"
wire_api = "responses"
env_key = "CONCLAVA_API_KEY"
...
```bash
export CONCLAVA_API_KEY=local
```

Use `conclava-full` for high-value review tasks and `conclava-heavy` for ds4-backed long-context work.
...
```text
claude-conclava-fast
claude-conclava-full
claude-conclava-heavy
```
...
Use `conclava-fast` for coding-agent style tasks, `conclava-full` for review, and `conclava-heavy` for ds4-backed heavy work. Use `conclava-vision-pro` for direct screenshot/OCR evidence, `conclava-agentic-pro` for qwen3.6 coding/thinking agent work, and `conclava-hermes-pro` for Hermes Desktop pro multimodal chats.

Optional MLX aliases are available for explicit testing only: `conclava-agentic-mlx` and `conclava-formatter-mlx`.
...
curl http://127.0.0.1:8088/v1/responses \
  -H "Content-Type: application/json" \
  -d '{"model":"conclava-fast","input":"用一句話說明 Conclava","stream":false}'
```
...
curl http://127.0.0.1:8088/v1/responses \
  -H "Content-Type: application/json" \
  -d '{"model":"conclava-fast","input":"請讀取 README.md","tools":[{"type":"function","name":"read_file","description":"Read a local file","parameters":{"type":"object","properties":{"path":{"type":"string"}},"required":["path"]}}],"stream":false}'
```
...
curl http://127.0.0.1:8088/v1/messages \
  ...
  -d '{"model":"claude-conclava-fast","max_tokens":1024,"tools":[{"name":"Bash","description":"Run a shell command","input_schema":{"type":"object","properties":{"command":{"type":"string"}},"required":["command"]}}],"messages":[{"role":"user","content":"請列出目前目錄"}],"stream":false}'
```
...
curl http://127.0.0.1:8088/v1/chat/completions \
  ...
  -d '{"model":"conclava-chat","messages":[{"role":"user","content":"用三點說明你是什麼。"}],"stream":false}'
```
...
curl http://127.0.0.1:8088/v1/responses \
  -H "Content-Type: application/json" \
  -d '{"model":"conclava-heavy","input":"設計如何分析整個 codebase 的依賴與風險。","metadata":{"debug":true},"stream":false}'
```
...
```text
conclava-agentic-mlx -> MODEL_AGENTIC_MLX
conclava-formatter-mlx -> MODEL_FORMATTER_MLX
claude-conclava-agentic-mlx -> MODEL_AGENTIC_MLX
claude-conclava-formatter-mlx -> MODEL_FORMATTER_MLX
```

The optional MLX aliases are explicit test/pro profiles only. `conclava-fast`, `conclava`, `conclava-chat`, and the standard Claude aliases keep the non-MLX default model plan.
