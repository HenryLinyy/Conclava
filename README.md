...
[![CI](https://github.com/HenryLinyy/conclava/actions/workflows/ci.yml/badge.svg)](https://github.com/HenryLinyy/conclava/actions/workflows/ci.yml)
...
Cloud providers hide all of that behind one smart endpoint. Conclava brings that same experience to your own machine:
...
# models: claude-conclava-fast | -full | -heavy | -fusion
```
...
This is the feature worth the repo. Call the `*-fusion` model and Conclava runs a **panel of models**, then a **judge** synthesizes their answers into a structured verdict:
...
A single self-contained `conclava/web/dashboard.html` streams the deliberation in real time — panel start/done events, judge tokens, and the final synthesis — so you can actually watch the council think instead of staring at a spinner.
