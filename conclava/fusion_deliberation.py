...
from conclava.fusion_schemas import PanelResponse, SynthesisInput
from conclava.fusion_synthesis import (
    build_synthesis_prompt,
    parse_structured_output,
)
from conclava.prompts import FUSION_AGENT_ANALYSIS_SYSTEM
from conclava.fusion_retry import chat_with_retry
...
logger = logging.getLogger("conclava.fusion_deliberation")


def _retry_settings() -> tuple[int, float]:
    try:
        import conclava.server as server

        config = getattr(server, "config", None)
        return (
            getattr(config, "fusion_max_retries", 0),
            getattr(config, "fusion_retry_base_delay", 1.0),
        )
    except Exception:
        return 0, 1.0
...
async def run_fusion_agent(
    *,
    ...
    panel_responses = run_panel_serial(
        preset=preset,
    ...
    for judge_model_id, backend in judge_candidates:
        try:
            if backend == "ds4":
                judge_response = ds4_client.chat_completion(
                    model=ds4_model,
                ...
                retry_cfg, retry_delay = _retry_settings()
                judge_response = chat_with_retry(
