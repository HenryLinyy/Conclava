...
class FusionConfig(BaseSettings):
    ...
    # Server
    conclava_host: str = "127.0.0.1"
    conclava_port: int = 8088
    conclava_default_profile: str = "fast-agent"
    conclava_timeout_seconds: int = 900
    conclava_queue_timeout_seconds: int = 5
    conclava_max_concurrent_requests: int = 1
    ...
    prefer_mlx_formatter: bool = Field(
        default=True, validation_alias="CONCLAVA_PREFER_MLX_FORMATTER"
    )
    mlx_formatter_max_chars: int = Field(
        default=1000, validation_alias="CONCLAVA_MLX_FORMATTER_MAX_CHARS"
    )
    ...
    @classmethod
    def _expand_lmstudio_cli_path(cls, v: str) -> str:
        """Expand ~ so .env values like ~/.lmstudio/bin/lms resolve on any machine."""
