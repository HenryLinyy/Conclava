...
[English guide](INSTALL.en.md) · [繁體中文](INSTALL.md)

> **Conclava** 是一個 OpenRouter-style multi-model deliberation router。
...
**預期**：`290 passed`（或更新版數量）— 大約 5-8 秒完成。
...
```bash
# 1. 編輯 Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist
#    把 /Users/yourname/Documents/conclava 換成專案實際路徑
#    例如:
#      /Users/yourname/conclava
#      /Users/yourname/.lmstudio/bin/lms
#      /Users/yourname/.local/share/...

# 2. 複製 plist 到 ~/Library/LaunchAgents/
cp Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist ~/Library/LaunchAgents/

# 3. 載入（id 是 plist filename 去 .plist）
launchctl load ~/Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist

# 4. 確認在跑
launchctl list | grep conclava
# 應該看到:
#   45157  -  io.github.henrylinyy.conclava.gateway
#   57174  0  io.github.henrylinyy.conclava.warmup.quality
#   57179  0  io.github.henrylinyy.conclava.warmup.budget
```

> launchd plist 裡用 `/Users/yourname/...` 是故意的 — 每台 Mac 的路徑不同，
...
把 `http://127.0.0.1:8088/v1` 加到 Hermes 的 provider 清單，model 填 `conclava-fusion` 或 `claude-conclava-fusion`。
...
```bash
# 看 log
tail -50 ~/Library/Logs/conclava-gateway.err.log
...
# 3. Python 找不到 conclava
source .venv/bin/activate
python3 -c "import conclava; print(conclava.__file__)"
```
...
tail -30 ~/Library/Logs/conclava-gateway.err.log

# 重啟
kill -TERM $(pgrep -f "uvicorn conclava.server")
...
```bash
# 啟動 / 停止
launchctl start io.github.henrylinyy.conclava.gateway
launchctl stop io.github.henrylinyy.conclava.gateway
launchctl kickstart -k io.github.henrylinyy.conclava.gateway   # 強制重啟

# 看 log
tail -f ~/Library/Logs/conclava-gateway.out.log
tail -f ~/Library/Logs/conclava-gateway.err.log
...
│   └── ...                     ← (其他輔助模組)
├── conclava_sdk/           ← Python client library
│   ├── __init__.py
│   ├── client.py               ← ConclavaClient
...
│   └── ...                     ← (其他輔助 script)
├── tests/                      ← 290 個 unit tests
│   ├── test_fusion_*.py
│   ├── test_conclava_sdk.py
│   ├── test_mlx_optimizations.py
│   ├── test_conversation_store.py
│   └── ...                     ← (其他測試)
├── Library/LaunchAgents/       ← launchd plist templates
│   ├── io.github.henrylinyy.conclava.gateway.plist
│   ├── io.github.henrylinyy.conclava.warmup.quality.plist
│   └── io.github.henrylinyy.conclava.warmup.budget.plist
└── .github/workflows/          ← CI
    └── ci.yml
```
...
| `CONCLAVA_WARMUP_ON_BOOT` | 是否要開機自動預載 |
| launchd plist 內 `/Users/yourname/...` | 你的絕對路徑 |
...
6. **CI** — `.github/workflows/ci.yml` 跑 290 個 tests 自動驗證
...
3. 查 `~/Library/Logs/conclava-gateway.err.log`
