"""Regression tests for issues found by the conclava-audit workflow.
...
from conclava.patch_protocol import is_test_command, validate_tool_call
from conclava.tool_validation import validate_tool_call as validate_args
from conclava.vision import image_from_url_value
...
def test_extract_fusion_request_ignores_bad_types():
    r = extract_fusion_request(
        {"fusion": {"analysis_models": "qwen", "judge_model": 5, "preset": 7}}
    )
...
def test_image_from_url_value_accepts_empty_and_parameterized_media_type():
    ...
    img2 = image_from_url_value(
        "openai_responses", "data:image/png;charset=utf-8;base64,QUJD"
    )
