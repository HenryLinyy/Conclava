...
def test_env_example_documents_all_runtime_knobs():
    ...
    required_keys = [
        ...
        "LMSTUDIO_CLI_PATH=~/.lmstudio/bin/lms",
        ...
        "MODEL_VISION_FAST=qwen/qwen3-vl-30b",
        "MODEL_VISION_PRO=qwen/qwen3-vl-30b",
        "MODEL_AGENTIC_PRO=google/gemma-4-26b-a4b-qat",
        "MODEL_HERMES_PRO=google/gemma-4-26b-a4b-qat",
        "MODEL_AGENTIC_MLX=google/gemma-4-26b-a4b-qat",
...
def test_helper_scripts_are_present_and_configurable():
    ...
    assert "--host ${CONCLAVA_HOST:-127.0.0.1}" in start_server
    assert "--port ${CONCLAVA_PORT:-8088}" in start_server
