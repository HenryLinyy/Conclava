...
def test_v18_qwable_model_defaults():
    """Qwable default model id matches the v1.8 plan."""
    cfg = FusionConfig()
...
def test_v18_vision_fast_uses_vision_capable_model():
    """vision-fast must be image-capable; Gemma text model 500s on image input."""
    cfg = FusionConfig()
    assert cfg.model_vision_fast == "qwen/qwen3-vl-30b"


def test_v18_agentic_mlx_budget_covers_qwen36_thinking():
    """qwen3.6 needs ~800 tokens before content appears on short replies."""
    cfg = FusionConfig()
