...
def test_build_pack_uses_task_text_and_tool_results_as_evidence():
    ...
    task = _task(
        "Fix conclava/server.py using tests/test_server.py",
        tool_results=[
            ToolResult(
                tool_call_id="call_1",
                name="list_files",
                content="conclava/config.py\nREADME.md",
                is_error=False,
                source_protocol="openai_responses",
                raw={},
            )
    ...
    assert "Fix conclava/server.py" in pack.raw_evidence[0]
...
def test_build_pack_adds_error_risk_and_vision_evidence():
    ...
    from conclava.context_compactor import ContextCompactor
    from conclava.schemas import ToolResult
    from conclava.vision import VisionEvidence
    ...
    task = _task(
        ...
        tool_results=[
            ToolResult(
                tool_call_id="call_2",
                name="read_file",
                content="permission denied for conclava/server.py",
                is_error=True,
                source_protocol="openai_responses",
                raw={},
            )
