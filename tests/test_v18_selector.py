...
from conclava.model_capabilities import RoleCapabilityError
from conclava.model_roles import ModelRole, WorkflowStage
from conclava.model_selector import ModelSelector, SelectedModel
...
def test_v17_select_uses_qwable_for_executor_when_enabled():
    """The live v1.7 orchestrator still calls select(workflow, 'executor').

    v1.8 must therefore route the preserved v1.7 API through the same Qwable
    primary chain; otherwise /health/models says Qwable is primary but runtime
    still tries qwen3-coder-next first (too large to load with planner resident).
    """
    cfg = FusionConfig()
    cfg.enable_qwable_executor = True
    selection = ModelSelector(cfg).select("agentic-workflow", "executor")
    assert selection.model == cfg.model_qwable
    assert selection.fallback_chain[0] == cfg.model_qwable
    assert cfg.model_coder in selection.fallback_chain


def test_v17_select_uses_qwable_for_repair_when_enabled():
    cfg = FusionConfig()
    cfg.enable_qwable_executor = True
    selection = ModelSelector(cfg).select("coding-workflow", "repair")
    assert selection.model == cfg.model_qwable
    assert selection.fallback_chain[0] == cfg.model_qwable
    assert cfg.model_coder in selection.fallback_chain
...
def test_capability_gate_blocks_qwable_for_judge_stage():
    ...
    from conclava.model_capabilities import build_qwable_spec, assert_model_allowed_for_role
