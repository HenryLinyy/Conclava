...
def _task(
    text="Implement conclava/agent_orchestrator.py",
    *,
    raw_request=None,
    tool_results=None,
):
    ...
    return {
        ...
        "usage": {
            ...
            "completion_tokens_details": {
                "reasoning_tokens": 600 if not content else 100
            },
        },
    }
...
async def test_run_uses_model_selector_for_planner_and_executor(tmp_path):
    ...
    assert compactor.calls == [
        ("Implement conclava/agent_orchestrator.py", "coding-workflow")
    ]
...
async def test_reasoning_model_planner_output_in_reasoning_content_is_accepted(
    tmp_path,
):
...
async def test_coding_workflow_advances_to_next_step_after_tool_result(tmp_path):
    ...
    first = await orchestrator.run(
        _task("Inspect and patch conclava/server.py"), "coding-workflow"
    )
...
async def test_coding_workflow_blocks_when_tool_call_limit_would_be_exceeded(tmp_path):
    ...
    action = await orchestrator.run(
        _task("Inspect conclava/server.py"), "coding-workflow"
    )
...
async def test_review_workflow_uses_reviewer_judge_finalizer_without_patch_loop(
    ...
    action = await orchestrator.run(
        _task("Review conclava/server.py"), "review-workflow"
    )
