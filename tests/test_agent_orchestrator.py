...
def _task(text="Implement conclava/agent_orchestrator.py", *, raw_request=None, tool_results=None):
...
class FakeModelClient:
    ...
    def chat_completion(self, *, model, messages, max_tokens, temperature):
        ...
        # 1) string → wrap in {"content": str} (legacy)
        # 2) dict → use as-is (full OpenAI/LM Studio response with choices, reasoning_content, etc.)
        if isinstance(response, str):
            return {"content": response}
        return response


def _lmstudio_reasoning_response(content, reasoning_text):
    """Build a realistic LM Studio response for a reasoning model.

    Reasoning models (qwen3.6-35b-a3b, deepseek-r1-distill-*) return their
    final structured output either in `content` (when tokens remain after
    thinking) or in `reasoning_content` (when thinking consumes the full
    budget). Gateway must accept BOTH.
    """
    return {
        "id": "chatcmpl-test",
        ...
        "usage": {
            ...
            "total_tokens": 1275,
            "completion_tokens_details": {"reasoning_tokens": 600 if not content else 100},
        },
    }
...
class FakeSelector:
    ...
    def select(self, workflow, stage, *, temperature=0.2):
        from conclava.model_roles import ModelRole, RoleSelection
...
def _tool_result(name, content, *, is_error=False):
    from conclava.schemas import ToolResult
...
def _saved_testing_run(store):
    from conclava.agent_state import AgentRun, AgentStep
...
def test_extract_agent_run_id_accepts_valid_metadata_only():
    from conclava.agent_orchestrator import extract_agent_run_id
...
async def test_run_uses_model_selector_for_planner_and_executor(tmp_path):
    orchestrator, client, selector, store, compactor = _orchestrator(
        ...
        [
            _planner_json(),
            _plan_critic_json(),
            json.dumps({"tool_call": {"name": "read_file", "input": {"path": "conclava/server.py"}}}),
        ],
    )
    ...
    assert action.tool_input == {"path": "conclava/server.py"}
    ...
    assert compactor.calls == [("Implement conclava/agent_orchestrator.py", "coding-workflow")]
...
async def test_reasoning_model_planner_output_in_reasoning_content_is_accepted(tmp_path):
    """RED test for the v1.7 planner-parse bug.

    LM Studio reasoning models (qwen3.6-35b-a3b, deepseek-r1-distill-*)
    return their final structured output in `message.reasoning_content`
    when chain-of-thought consumes the entire max_tokens budget — in which
    case `message.content` is empty. The gateway's _response_content() only
    reads `content`, so it returns "" and the orchestrator fails with
    `planner_json_parse_failed`.

    This test reproduces that exact response shape (planner + plan_critic)
    using the coding-workflow to assert the planner succeeds and the
    workflow advances to the executor stage.
    """
    plan_json = _planner_json()
    reasoning_planner_response = _lmstudio_reasoning_response(
        content="",  # empty: thinking ate the budget
        reasoning_text=f"```json\n{plan_json}\n```",  # JSON plan in reasoning block
    )
    # After planner succeeds, coding-workflow runs plan_critic, then executor.
    critic_json = _plan_critic_json(fatal=False)
    critic_response = _lmstudio_reasoning_response(
        content=critic_json,
        reasoning_text="plan looks fine",
    )
    # Executor issues a tool_call (read_file) — this proves the workflow
    # advanced past planner + critic stages.
    executor_response = _lmstudio_reasoning_response(
        content=json.dumps({"tool_call": {"name": "read_file", "input": {"path": "conclava/server.py"}}}),
        reasoning_text="I should read the server.py file",
    )
    orchestrator, _client, _selector, _store, _compactor = _orchestrator(
        tmp_path, [reasoning_planner_response, critic_response, executor_response]
    )

    action = await orchestrator.run(_task(), "coding-workflow")

    # The decisive check: workflow advanced past planner to executor stage
    # and the executor produced a tool_call. This is only possible if the
    # planner successfully parsed the JSON from reasoning_content.
    assert action.type == "tool_call", (
        f"expected tool_call from executor, got {action.type!r} with text {action.text!r}"
    )
    assert action.tool_name == "read_file"
    assert action.tool_input == {"path": "conclava/server.py"}
...
async def test_tool_result_continuation_appends_evidence_and_requests_coding_tests(tmp_path):
    ...
    first = await orchestrator.run(
        _task("Inspect conclava/server.py. Run tests: python -m pytest tests/test_agent_orchestrator.py"),
        "coding-workflow",
    )

    continuation = await orchestrator.run(
        _task(
            "Inspect conclava/server.py. Run tests: python -m pytest tests/test_agent_orchestrator.py",
...
async def test_coding_workflow_advances_to_next_step_after_tool_result(tmp_path):
    ...
    first = await orchestrator.run(_task("Inspect and patch conclava/server.py"), "coding-workflow")

    second = await orchestrator.run(
        _task(
            "Inspect and patch conclava/server.py",
            raw_request={"metadata": {"agent_run_id": first.trace["agent_run_id"]}},
...
async def test_coding_workflow_blocks_when_tool_call_limit_would_be_exceeded(tmp_path):
    ...
    action = await orchestrator.run(_task("Inspect conclava/server.py"), "coding-workflow")
...
async def test_coding_workflow_requires_tests_after_mutating_step(tmp_path):
    ...
    first = await orchestrator.run(
        _task("Patch conclava/server.py and tests/test_agent_orchestrator.py"),
        "coding-workflow",
    )

    continuation = await orchestrator.run(
        _task(
            "Patch conclava/server.py and tests/test_agent_orchestrator.py",
            raw_request={"metadata": {"agent_run_id": first.trace["agent_run_id"]}},
...
async def test_review_workflow_uses_reviewer_judge_finalizer_without_patch_loop(tmp_path):
    ...
    action = await orchestrator.run(_task("Review conclava/server.py"), "review-workflow")
