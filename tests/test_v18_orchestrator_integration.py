...
async def test_v18_code_agent_trace_records_qwable_executor(tmp_path):
    """v1.8 stage plumbing: Qwable is the runtime executor primary
    for both the explicit v1.8 stage API and the preserved v1.7 select() API.
    """
    from conclava.config import FusionConfig
    cfg = FusionConfig(
    ...
    # v1.7 select() is preserved as an API, but it now routes executor
    # through the v1.8 Qwable primary chain as well.
    v17 = sel.select("coding-workflow", "executor")
    assert v17.role == ModelRole.EXECUTOR
    assert v17.model == cfg.model_qwable
