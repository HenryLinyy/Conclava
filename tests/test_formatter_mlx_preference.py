...
Conditions to route to formatter-mlx:
  ...
  - profile == "fast-agent"
  - task.tools is empty
  - task.text length < 1000 chars (CONCLAVA_MLX_FORMATTER_MAX_CHARS)
...
from conclava.models import DS4Client, OllamaClient
...
def _make_config(**overrides) -> FusionConfig:
    defaults = dict(
        ...
        conclava_timeout_seconds=60,
        conclava_queue_timeout_seconds=5,
        conclava_max_concurrent_requests=1,
...
def _make_task(text: str, tools: list | None = None) -> ParsedAgentTask:
    from conclava.schemas import ToolSpec
...
def test_fusion_config_env_override_prefer_mlx_formatter(monkeypatch):
    """CONCLAVA_PREFER_MLX_FORMATTER=false env var should disable."""
    monkeypatch.setenv("CONCLAVA_PREFER_MLX_FORMATTER", "false")
    config = FusionConfig()
    assert config.prefer_mlx_formatter is False


def test_fusion_config_env_override_max_chars(monkeypatch):
    """CONCLAVA_MLX_FORMATTER_MAX_CHARS env var should set threshold."""
    monkeypatch.setenv("CONCLAVA_MLX_FORMATTER_MAX_CHARS", "500")
    config = FusionConfig()
