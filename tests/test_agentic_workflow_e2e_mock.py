...
from conclava.schemas import ParsedAgentTask, ToolResult
...
async def test_rejected_shell_tool_call_returns_step_result_guidance_not_rejected_error(tmp_path):
    ...
    action = await core.execute(_task("agentic-workflow", "check Documents usage"))

    assert action.type == "final_answer"
    assert "tool_call_rejected" not in (action.text or "")
    payload = json.loads(action.text)
    assert payload["step_result"]["status"] == "done"
    assert "du -sh ~/Documents" in payload["step_result"]["content"]


@pytest.mark.asyncio
async def test_malformed_executor_tool_call_input_does_not_raise_500(tmp_path):
    core, _fake_client = _core(
        tmp_path,
        [
            fake_planner_response(),
    ...
    action = await core.execute(_task("coding-workflow", "write a small function"))

    assert action.type == "final_answer"
    assert "tool_call input must be an object" not in (action.text or "")
    payload = json.loads(action.text)
    assert payload["step_result"]["status"] == "done"
...
async def test_test_failure_uses_repair_model_and_returns_patch_tool_call(tmp_path):
    core, fake_client = _core(
        ...
        [
            fake_executor_tool_call_response(
                name="apply_patch",
                tool_input={
                    "path": "conclava/agent_orchestrator.py",
                    "patch": "*** Begin Patch\n*** End Patch\n",
                },
            )
        ],
    ...
    assert [call["model"] for call in fake_client.calls] == [core.config.model_qwable]
