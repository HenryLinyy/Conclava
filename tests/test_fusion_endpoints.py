"""G10: Endpoint tests for conclava-fusion / claude-conclava-fusion.
...
def _make_client(mock_core):
    import conclava.server as server_mod
...
def test_openai_chat_endpoint_accepts_conclava_fusion():
    """POST /v1/chat/completions with model=conclava-fusion returns 200."""
    trace = _fusion_trace()
    ...
    assert data["model"] == "conclava-fusion"
...
def test_openai_responses_endpoint_accepts_conclava_fusion():
    """POST /v1/responses with model=conclava-fusion returns 200."""
    mock = _make_mock_fusion_core(
...
def test_anthropic_endpoint_accepts_claude_conclava_fusion():
    """POST /v1/messages with model=claude-conclava-fusion returns 200."""
    mock = _make_mock_fusion_core(
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["model"] == "claude-conclava-fusion"
