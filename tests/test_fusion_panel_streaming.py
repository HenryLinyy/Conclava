...
from conclava.fusion_deliberation import _run_panel_serial_streaming
...
@pytest.mark.asyncio
async def test_panel_falls_back_to_non_streaming_client():
    ...
    items = [
        ...
        async for item in _run_panel_serial_streaming(
            preset=preset,
            original_prompt="x",
            panel_client=NonStreamingClient(),
            panel_max_tokens=500,
            temperature=0.3,
        )
    ]
    response = next(item for item in items if not hasattr(item, "event"))
