...
def test_openai_responses_agent_aliases():
    ...
    assert (
        resolve_profile("conclava-code-agent", "openai_responses") == "coding-workflow"
    )
    assert (
        resolve_profile("conclava-review-agent", "openai_responses")
        == "review-workflow"
    )
...
def test_anthropic_agent_aliases():
    assert (
        resolve_profile("claude-conclava-agent", "anthropic_messages")
        == "agentic-workflow"
    )
    assert (
        resolve_profile("claude-conclava-code-agent", "anthropic_messages")
        == "coding-workflow"
    )
    assert (
        resolve_profile("claude-conclava-review-agent", "anthropic_messages")
        == "review-workflow"
    )
