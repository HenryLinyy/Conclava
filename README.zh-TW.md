...
雲端廠商把這一切藏在一個聰明的 endpoint 後面。Conclava 把同樣的體驗帶到你自己的機器上：
...
# 模型：claude-conclava-fast | -full | -heavy | -fusion
```
...
這是值得這個 repo 的功能。呼叫 `*-fusion` 模型，Conclava 會跑一個 **模型 panel**，再由一個 **judge** 把它們的答案統整成結構化判斷：
...
單一自包含的 `conclava/web/dashboard.html` 會即時串流整個 deliberation——panel 開始／完成事件、judge token、最終統整——讓你真的看著 council 思考，而不是盯著轉圈圈。
