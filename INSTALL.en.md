...
Conclava is built for Apple Silicon and uses LM Studio as its local OpenAI-compatible model backend.
...
```bash
git clone https://github.com/HenryLinyy/conclava.git
cd conclava
...
- Conclava at `http://127.0.0.1:8088`
- LM Studio CLI at `~/.lmstudio/bin/lms`

Edit `.env` if your ports, model IDs, or CLI path differ. Conclava reads this file automatically when started from the repository directory.
...
Use `claude-conclava-fast`, `claude-conclava-full`, `claude-conclava-heavy`, or `claude-conclava-fusion`.
...
sed \
  -e "s|/Users/yourname/Documents/conclava|$(pwd)|g" \
  -e "s|/Users/yourname|$HOME|g" \
  Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist \
  > ~/Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist

launchctl load -w ~/Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist
```
...
```bash
launchctl unload ~/Library/LaunchAgents/io.github.henrylinyy.conclava.gateway.plist
launchctl unload ~/Library/LaunchAgents/io.github.henrylinyy.conclava.warmup.quality.plist
launchctl unload ~/Library/LaunchAgents/io.github.henrylinyy.conclava.warmup.budget.plist
```
