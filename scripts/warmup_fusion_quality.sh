...
# G12: Add CONCLAVA_WARMUP_ON_BOOT opt-out.
...
if [ "${CONCLAVA_WARMUP_ON_BOOT:-true}" != "true" ]; then
    [ "$QUIET" != "1" ] && echo "[warmup_fusion_quality] CONCLAVA_WARMUP_ON_BOOT!=true, skipping"
    exit 0
fi
...
try:
    from conclava.mlx_optimizations import apply_recommended_settings
