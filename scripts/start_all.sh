...
#   CONCLAVA_HOST / CONCLAVA_PORT   gateway bind (預設 127.0.0.1:8088)
...
CONCLAVA_HOST="${CONCLAVA_HOST:-127.0.0.1}"
CONCLAVA_PORT="${CONCLAVA_PORT:-8088}"
...
# ---- 3/3 gateway ------------------------------------------------------------
echo "[3/3] Conclava gateway (:${CONCLAVA_PORT})"
if port_up "${CONCLAVA_PORT}"; then
  echo "  • 已在執行,略過"
else
  echo "  • start_server.sh (log: ${LOG_DIR}/gateway.log) ..."
  nohup bash "${SCRIPT_DIR}/start_server.sh" >"${LOG_DIR}/gateway.log" 2>&1 &
  wait_for_port "${CONCLAVA_PORT}" "gateway" 60
fi
wait_for_http "http://${CONCLAVA_HOST}:${CONCLAVA_PORT}/health" "gateway" 30
...
printf '  %-10s :%-5s %s\n' "gateway"  "${CONCLAVA_PORT}" "$(port_up "${CONCLAVA_PORT}" && echo UP || echo DOWN)"
echo
echo "log: ds4 -> ${LOG_DIR}/ds4.log"
echo "     gateway -> launchd 託管時看 ~/Library/Logs/conclava-gateway.{out,err}.log"
