...
case "$PRESET" in
    quality)
        ...
        launchctl start io.github.henrylinyy.conclava.warmup.quality 2>/dev/null || true
        ;;
    budget)
        bash "${SCRIPT_DIR}/warmup_fusion_budget.sh" --quiet
        launchctl start io.github.henrylinyy.conclava.warmup.budget 2>/dev/null || true
        ;;
    *)
