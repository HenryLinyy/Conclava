#!/usr/bin/env bash
# Conclava — one-shot verification of all features / profiles / workflows.
#
...
miss=[p for p in need if ('conclava'+p) not in ids]
miss+=['claude-conclava-fast'] if 'claude-conclava-fast' not in ids else []
...
hdr "2. Protocols"
check_chat "OpenAI Chat" "conclava-fast" "Reply with exactly: PONG"
# responses
curl -s --max-time 300 "$BASE/v1/responses" -H "Content-Type: application/json" \
  -d '{"model":"conclava-fast","input":"Reply with exactly: PONG","max_output_tokens":40}' \
...
curl -s --max-time 300 "$BASE/v1/messages" -H "Content-Type: application/json" \
  -d '{"model":"claude-conclava-fast","max_tokens":40,"messages":[{"role":"user","content":"Reply with exactly: PONG"}]}' \
...
curl -s --max-time 30 "$BASE/v1/messages/count_tokens" -H "Content-Type: application/json" \
  -d '{"model":"claude-conclava-fast","messages":[{"role":"user","content":"hi"}]}' \
  | grep -q 'input_tokens' && ok "count_tokens" || bad "count_tokens"

hdr "3. Text / agentic profiles (smoke)"
check_chat "fast"          "conclava-fast"          "Say PONG"
check_chat "full"          "conclava-full"          "Say PONG" 1500   # panel+judge: needs room
check_chat "chat"          "conclava-chat"          "Say PONG"
check_chat "agentic-pro"   "conclava-agentic-pro"   "Say PONG"
check_chat "hermes-pro"    "conclava-hermes-pro"    "Say PONG"
check_chat "agentic-mlx"   "conclava-agentic-mlx"   "Say PONG"
check_chat "formatter-mlx" "conclava-formatter-mlx" "Say PONG"
...
hbody=$(curl -s --max-time 300 "$BASE/v1/chat/completions" -H "Content-Type: application/json" \
  -d '{"model":"conclava-heavy","messages":[{"role":"user","content":"Say PONG"}],"max_tokens":64}')
...
body={"model":"conclava-vision-fast","max_tokens":30,"messages":[{"role":"user","content":[
...
fbody=$(curl -s --max-time 480 "$BASE/v1/chat/completions" -H "Content-Type: application/json" \
  -d '{"model":"conclava-fusion","fusion":{"preset":"budget"},"messages":[{"role":"user","content":"In one sentence, what is 2+2?"}]}')
...
if [ "$FULL" = "1" ]; then
  for wf in agent code-agent review-agent; do
    wbody=$(curl -s --max-time 900 "$BASE/v1/chat/completions" -H "Content-Type: application/json" \
      -d "{\"model\":\"conclava-$wf\",\"messages\":[{\"role\":\"user\",\"content\":\"Write a python one-liner add(a,b) that returns a+b. Keep it short.\"}]}")
