...
GATEWAY_LAUNCHD_LABEL="${GATEWAY_LAUNCHD_LABEL:-io.github.henrylinyy.conclava.gateway}"
...
if [[ "${STOP_GATEWAY}" -eq 1 ]]; then
  if launchd_loaded "${GATEWAY_LAUNCHD_LABEL}"; then
    ...
    for _ in $(seq 1 10); do
      lsof -nP -iTCP:"${CONCLAVA_PORT}" -sTCP:LISTEN >/dev/null 2>&1 || { echo "    ✓ gateway 已停"; break; }
      sleep 1
    done
    echo "    ↩ 要恢復: launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/${GATEWAY_LAUNCHD_LABEL}.plist"
  else
    stop_port "${CONCLAVA_PORT}" "gateway"
  fi
else
  if launchd_loaded "${GATEWAY_LAUNCHD_LABEL}"; then
    echo "  • gateway (:${CONCLAVA_PORT}) 由 launchd 託管,kill 會被自動重啟 -> 保留 (要停請加 --gateway)"
  else
    echo "  • gateway (:${CONCLAVA_PORT}) 保留 (要停請加 --gateway)"
  fi
fi
