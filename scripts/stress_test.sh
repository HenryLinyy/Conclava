...
fusion() { run "fusion:$1" "{\"model\":\"conclava-fusion\",\"fusion\":{\"preset\":\"$1\"},\"messages\":[{\"role\":\"user\",\"content\":\"$2\"}]}" "${3:-720}"; }
wf()     { run "workflow:$1" "{\"model\":\"conclava-$1\",\"messages\":[{\"role\":\"user\",\"content\":\"$2\"}]}" 900; }

echo "=== Conclava stress test @ $B (min_free=${MIN_FREE_PCT}%) ==="
