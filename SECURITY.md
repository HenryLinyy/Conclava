...
Conclava is designed to bind to `127.0.0.1`. It has no authentication layer and must not be exposed directly to an untrusted network.
...
- Conversations are stored under `~/.conclava/conversations` when persistence is used.
